...
def test_second_order_gradient():
    ...
    variance = 0.5
    x = tf.constant([1.0, 2.0, 3.0], dtype=tf.float64)
    y = tf.constant([1.0, 2.0, 3.0], dtype=tf.float64) + 1e-5 # This tolerance is needed because autodiff does not handle division by zero really well.
    ...
    print("Passed second order")
# Run the unit tests
test_first_order_gradient()
