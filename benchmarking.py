...
if __name__ == '__main__':
    ...
    new_not_worked = [4, 13, 21, 33, 41, 42, 43, 44, 46, 52, 53, 4, 13, 21, 37, 40, 42, 43, 53, 4, 13, 21, 41, 42, 43, 53, 4, 13, 16, 21, 33, 40, 42, 43, 44]
    going_through_floor = [33, 42, 44, 46, 52, 53, 37, 42, 53, 42, 53, 16, 33, 42, 44]
    # didnt_work = []
    # for i in range(55):
    #     for j in range(5):
    #         try:
    #             np.load("/home/freshpate/saved_paths/kuka/bookshelves/pair_{}_run_{}.npy".format(i, j))
    #         except:
    #             didnt_work.append((i, j))
    # print(len(didnt_work))
    # sys.exit()
    # print(idx)
    final = {4, 13, 21, 40, 43}
    start_joints = np.array(queries[0][0], dtype=np.float64).reshape(1, robot.dof)
    end_joints = np.array(queries[0][1], dtype=np.float64).reshape(1, robot.dof)
