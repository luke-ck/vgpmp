...
def solve_planning_problem(env, robot, sdf, start_joints, end_joints, robot_params, planner_params, scene_params,
    ...
    # Kuka
    # Bookshelves
    # the following is for any state that comes or goes from the bottom left corner of the bookshelf

    # q_mu = np.concatenate([[y[0] for _ in range(planner_params["num_inducing"] // 3)], [    [0.95010236, 0.9412263, 1.86583927, 2.06707834, -0.12104962, -0.41134678, 2.38845786] for _ in range(planner_params["num_inducing"] // 3)], [y[1] for _ in range(planner_params["num_inducing"] // 3)]], axis=0)
    q_mu = np.concatenate([[y[0] for _ in range(planner_params["num_inducing"] // 3)], [    [-1.78810809, 0.17870216, -2.95667161, 1.6378849, -1.01828982, 0.0606072, 0.65012045] for _ in range(planner_params["num_inducing"] // 3)], [y[1] for _ in range(planner_params["num_inducing"] // 3)]], axis=0)
    ...
    if res == 1:
        ...
        np.save("/home/freshpate/saved_paths/angles/kuka/bookshelves/pair_{}_run_{}".format(k, run), best_sample)
