...
def create_planner(env, robot, sdf, start_joints, end_joints, robot_params, planner_params, scene_params,
...
def solve_planning_problem(env, robot, sdf, start_joints, end_joints, robot_params, planner_params, scene_params,
                           trainable_params, graphics_params, planner, weights, run=0, k=0):
    grid_spacing_X = planner_params["time_spacing_X"]
    grid_spacing_Xnew = planner_params["time_spacing_Xnew"]
    dof = robot_params["dof"]
    num_steps = planner_params["num_steps"]
    ...
    # q_mu = np.concatenate([[y[0] for _ in range(planner_params["num_inducing"] // 3)], [[0.34106401, -0.51710239, -0.07689295, 1.68875076, -0.77060238, 0.41600313, 2.25305135] for _ in range(planner_params["num_inducing"] // 3)], [y[1] for _ in range(planner_params["num_inducing"] // 3)]], axis=0)

    # Means for LAB
    #for not_working
    # q_mu = np.concatenate([[y[0] for _ in range(planner_params["num_inducing"] // 3)], [[-1.67437444, 1.20892299, 1.08881254, 2.37224745, -4.50290855, 0.29917642, 2.57918215] for _ in range(planner_params["num_inducing"] // 3)], [y[1] for _ in range(planner_params["num_inducing"] // 3)]], axis=0)
    
    
    # q_mu = np.concatenate([[y[0] for _ in range(planner_params["num_inducing"] // 3)], [(y[0] + y[1]) / 2 for _ in range(planner_params["num_inducing"] // 3)], [y[1] for _ in range(planner_params["num_inducing"] // 3)]], axis=0)
    ...
    if graphics_params["visualize_ee_path_uncertainty"]:
        ...
        for joint_config in sample_mean:
            ...
            #     p.addUserDebugLine(prev_xx, xx, rgbaColor=[1, 0, 0, 1], 
            #                        lineWidth=5.0, lifeTime=0, physicsClientId=env.sim.physicsClient)
    ...
    # if res == 1:
        ...
        # np.save("/home/freshpate/saved_paths/angles/wam/lab/pair_{}_run_{}".format(k, run), best_sample)
    return res
